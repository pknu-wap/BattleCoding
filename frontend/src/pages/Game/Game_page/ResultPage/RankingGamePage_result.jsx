...
function RankingResult() {
  ...
  const getDifficultyLabel = (level) => {
    return level === "EASY" ? "하" : level === "MEDIUM" ? "중" : level === "HARD" ? "상" : "-";
  };
  ...
  return (
    <div className="rankingResultWrapper">
      ...
      <div className="rankingResultSection">
        ...
        <div className="resultSumarry">
          <div className="summaryInfo">
            총 정답<span>{correctCount} / 10</span>
          </div>
          <div className="summaryInfo">
            획득 XP <span>{totalXp > 0 ? `+${totalXp}` : totalXp < 0 ? totalXp : "0"}</span>
          </div>
          <div className="summaryInfo">
            순위 <span>{rank}등</span>
          </div>
      ...
      <div className="buttonSection">
        ...
        <button onClick={() => navigate("/game/ranking")}>나가기</button>
      </div>
    </div>
  );
}
